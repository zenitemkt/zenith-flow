...
export async function POST(request: Request, { params }: RouteParams) {
  ...
  const estimatedMinutes = raw === null || raw === "" ? null : Number(raw);

  if (estimatedMinutes !== null && (!Number.isFinite(estimatedMinutes) || estimatedMinutes < 0)) {
    return NextResponse.json({ error: "Estimativa inválida." }, { status: 400 });
  }

  await prisma.task.update({ where: { id: task.id }, data: { estimatedMinutes } });
