...
export function EnpsResponseForm({ token, commentPrompt }: { token: string; commentPrompt: string | null }) {
  ...
  async function handleSubmit() {
    ...
    setError(null);
    setLoading(true);
    const response = await fetch(`/api/public/enps/${token}/respond`, {
      method: "POST",
  ...
  if (done) {
    return (
      <div className="rounded-lg bg-[#DCFCE7] p-4 text-center text-sm font-medium text-[#166534]">
        Obrigado! Sua resposta é anônima e não fica associada a você.
      </div>
    );
  }
    ...
    <div className="flex flex-col gap-4">
      ...
      {commentPrompt && (
        <div className="flex flex-col gap-1.5">
          <label htmlFor="enps-comment" className="text-sm font-medium text-[#344054]">
            {commentPrompt}
          </label>
          <textarea
            id="enps-comment"
      ...
      >
        {loading ? "Enviando..." : "Enviar resposta anônima"}
      </button>
    </div>
  );
}
