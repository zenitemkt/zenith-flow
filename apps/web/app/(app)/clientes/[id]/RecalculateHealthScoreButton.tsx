...
export function RecalculateHealthScoreButton({ clientId }: { clientId: string }) {
  ...
  async function handleClick() {
    setError(null);
    setLoading(true);
    const response = await fetch(`/api/clients/${clientId}/health-score/recalculate`, { method: "POST" });
    setLoading(false);
    if (!response.ok) {
      const body = await response.json().catch(() => null);
      setError(body?.error ?? "Não foi possível recalcular.");
      return;
    }
    router.refresh();
  }
