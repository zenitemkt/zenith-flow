...
export function EditCampaignForm({ campaignId, initial }: Props) {
  ...
  async function handleSubmit(event: FormEvent) {
    ...
    setError(null);
    setLoading(true);
    const response = await fetch(`/api/enps/campaigns/${campaignId}`, {
      method: "PATCH",
  ...
  return (
    <form onSubmit={handleSubmit} className="flex flex-col gap-3">
      <div className="flex flex-col gap-1.5">
        <label htmlFor="edit-enps-name" className="text-sm font-medium text-[#344054]">
          Nome da pesquisa
        </label>
        <input
          id="edit-enps-name"
      ...
      <div className="flex flex-col gap-1.5">
        <label htmlFor="edit-enps-header" className="text-sm font-medium text-[#344054]">
          Cabeçalho
        </label>
        <textarea id="edit-enps-header" value={headerText} onChange={(e) => setHeaderText(e.target.value)} className={TEXTAREA_CLASS} />
      </div>
      <div className="flex flex-col gap-1.5">
        <label htmlFor="edit-enps-question" className="text-sm font-medium text-[#344054]">
          Pergunta (0 a 10)
        </label>
        <textarea
          id="edit-enps-question"
          required
          value={question}
      ...
      <div className="flex flex-col gap-1.5">
        <label htmlFor="edit-enps-comment-prompt" className="text-sm font-medium text-[#344054]">
          Pergunta de comentário
        </label>
        <input
          id="edit-enps-comment-prompt"
          value={commentPrompt}
      ...
      <div className="flex flex-col gap-1.5">
        <label htmlFor="edit-enps-footer" className="text-sm font-medium text-[#344054]">
          Rodapé
        </label>
        <textarea id="edit-enps-footer" value={footerText} onChange={(e) => setFooterText(e.target.value)} className={TEXTAREA_CLASS} />
      </div>
