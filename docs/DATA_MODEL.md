...
## Portal do Cliente — boleto em PDF e tráfego pago — `FinanceEntry.boletoAssetId`, `Campaign.clientId`

Pedido do usuário, 2026-09-07 (verificado por leitura de código, não suposição, que essas eram as duas únicas peças faltando do Portal). Ver `docs/DECISIONS.md`.

- **`FinanceEntry.boletoAssetId` reaproveita `MediaAsset`/R2 sem tabela nova** — mesmo padrão de idempotência de `collectionTaskId` (`@unique`, `onDelete: SetNull`): um arquivo só pode ser o boleto de um lançamento, e apagar o arquivo nunca apaga o lançamento. A rota de vínculo (`POST /api/finance/entries/:id/boleto`) exige que `MediaAsset.clientId` bata com `FinanceEntry.clientId` — sem essa checagem, a rota de download (`GET /api/media/:id`, que já nega acesso de portal a arquivo de outro cliente) e o lançamento poderiam discordar sobre de quem é o arquivo.
- **`Campaign.clientId` é aditivo, não um modelo novo** — a mesma tabela da seção 36 serve dois propósitos pelo valor de uma coluna: nulo é o funil comercial da própria agência (atribuído a Lead/Opportunity, seção 39); preenchido é tráfego pago que a agência gerencia PRA aquele cliente, mostrado a ele em `/portal/trafego`. Nunca modelei como duas tabelas separadas porque a estrutura (canal, orçamento, métricas diárias) é idêntica — só o "de quem é" muda.
- **`/portal/trafego` é somente leitura e nunca mostra atribuição** — a seção "Atribuição Zenite" da página interna (`/comercial/campanhas/:id`) é sobre o funil comercial da própria agência; expor isso ao cliente não faz sentido nenhum e foi deliberadamente deixado de fora, reaproveitando só a seção "Métricas reportadas".
