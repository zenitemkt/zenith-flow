...
- **`/portal/trafego` é somente leitura e nunca mostra atribuição** — a seção "Atribuição Zenith" da página interna (`/comercial/campanhas/:id`) é sobre o funil comercial da própria agência; expor isso ao cliente não faz sentido nenhum e foi deliberadamente deixado de fora, reaproveitando só a seção "Métricas reportadas".
