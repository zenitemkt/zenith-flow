...
**Decisão 4 — `/portal/trafego` é somente leitura e nunca mostra o painel "Atribuição Zenith"**: atribuição (touchpoints, modelos, receita via `Opportunity`) é sobre o funil comercial da própria agência (seção 39) — não faz sentido nenhum pro cliente, que só quer ver o que foi investido/entregue na campanha dele. A tela reaproveita só a seção "Métricas reportadas" da página interna, sem o resto.
...
**Contexto**: última fatia da Fase 3 sem bloqueio de credencial externa — depois desta, tudo que resta (GTM/Ads, e-mail/WhatsApp, Zenith AI) depende de conta/infra de terceiro. A seção 40 pede um "motor de automações" completo (workflow visual, gatilhos de domínio, idempotência/retry/dead-letter reais) — várias peças desse pedido pressupõem infraestrutura que este projeto documentadamente não tem ainda (`apps/worker`, fila, um sistema formal de `outbox_events`). Implementei o subconjunto que dá pra construir com honestidade sobre o que falta.
...
**Decisão 5 — "Não confundir" (seção 36) implementado como duas seções visualmente separadas na página da campanha**: "Métricas reportadas" (o que foi digitado à mão, rotulado como tal) e "Atribuição Zenith" (calculada a partir de tracking+CRM próprios) nunca aparecem misturadas num único número — cada uma com seu próprio rótulo e uma frase explícita avisando que podem divergir, exatamente o texto de alerta da seção 36.
...
**Decisão 1 — tracking é sobre o site da própria agência, não de clientes dela**: a seção 39 (Leads/CRM) já modela o funil comercial da agência que usa o Zenith Flow; a seção 34 existe pra alimentar esse funil com visitantes do site de marketing da própria agência. Por isso `TrackingEvent`/`TrackingVisitor` são escopados por `agencyId` (não por `clientId`) e `TrackingVisitor.leadId` aponta pro `Lead` já existente da seção 39 — identificar um visitante por e-mail cria ou reaproveita um `Lead`, fechando um loop real com um módulo que já existia, em vez de inventar um "CRM de tracking" paralelo.
